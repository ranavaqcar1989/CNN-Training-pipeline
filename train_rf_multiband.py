...
class IntegratedSegmentor:
        ...
        if feature_num < len(json_data['features']):
            ...
            if x_max > x_min and y_max > y_min:
                ...
                if patch.size > 0:
                    ...
                    # Additional features for 5-band images
                    mean_bands = masked_image[mask == 255].mean(axis=0)
                    std_bands = masked_image[mask == 255].std(axis=0)
                    ...
                    # Store the features from all 5 bands
                    self.data.append([class_name, total_sum, pixel_mean, pixel_max, pixel_std,
                                      mean_bands[0], mean_bands[1], mean_bands[2], mean_bands[3], mean_bands[4],
                                      std_bands[0], std_bands[1], std_bands[2], std_bands[3], std_bands[4],
                                      area, aspect_ratio])
            ...
            if file.endswith('.tif'):
                ...
                if os.path.exists(image_filename) and os.path.exists(geojson_filepath):
                    ...
                    with rasterio.open(image_filename) as src:
                        ...
                        # Stack all 5 bands for pixel calculations
                        if image.shape[0] == 5:
                            image = np.stack([image[0], image[1], image[2], image[3], image[4]], axis=-1)
                        else:
                            print(f"Warning: {image_filename} does not have 5 bands.")
                            continue
    ...
    def csv_maker(self):
        ...
        columns = ['ClassName', 'PixelSum', 'Pixel Mean', 'Pixel Max', 'Pixel-std',
                   'mean_band1', 'mean_band2', 'mean_band3', 'mean_band4', 'mean_band5',
                   'std_band1', 'std_band2', 'std_band3', 'std_band4', 'std_band5', 'area', 'aspect_ratio']
        df = pd.DataFrame(self.data, columns=columns)
        df.to_csv(output_file, index=False)

    def random_forest_classifier(self):
        df = pd.read_csv(os.path.join(os.path.dirname(self.image_folder), "results", "train.csv"))
        feature_columns = ['PixelSum', 'Pixel Mean', 'Pixel Max', 'Pixel-std',
                           'mean_band1', 'mean_band2', 'mean_band3', 'mean_band4', 'mean_band5',
                           'std_band1', 'std_band2', 'std_band3', 'std_band4', 'std_band5', 'area']
